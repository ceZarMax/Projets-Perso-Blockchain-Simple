...
class Block{
    constructor(index, timestamp, data, previousHash = ''){
        ...
        this.data = data; // Toutes les datas et détails que l'on veut associer à ce block (Argent transféré, Émetteur, Récepteur)
...
console.log('\nMinage du block 1..');
ABChain.addBlock(new Block(1, "18/10/2022", "Send : 100 MXC to Papa")); // Ajouter un block à ma blockchain avec un data d'envoie de 100 MXC par exemple
console.log('\nMinage du block 2..');
ABChain.addBlock(new Block(2, "18/10/2022", "Receive : 100 MXC from Max")); // Ajouter un block à ma blockchain avec un data qui reçois 100 MXC
console.log('\nMinage du block 3..');
ABChain.addBlock(new Block(3, "18/10/2022", "Stake : 100 MXC from Max to ABChain Pool")); // Ajouter un block à ma blockchain avec un data qui reçois 100 MXC
// 4 zéros car la difficulté est de 4 et est plus sécurisé pour éviter les spams, les duplications..
// Ainsi, nous pouvons controler combien de nouveaux blocks peuvent etre crées.


//ABChain.chain[1].data = {amout: 1000}; // J'ai changé les donnés data du bloc 1 mais je n'ai pas recalculé le hash, donc, cela crée une erreur de lien entre deuc blocs
//ABChain.chain[1].hash = ABChain.chain[1].calculateHash(); // Modifier le bloc 1 dans la blockchain mais le lien avec le bloc suivant est rompu
// On ne peut ni altérer, ni supprimer un bloc dans une blockchain qui a un bloc qui lui succède.

console.log('\nLa blockchain est valide ? \n' + ABChain.isChainValid() + '\n');

console.log(JSON.stringify(ABChain, null, 4)); // Permettre de voir la blockchain sur notre terminal avec 4 espaces comme format